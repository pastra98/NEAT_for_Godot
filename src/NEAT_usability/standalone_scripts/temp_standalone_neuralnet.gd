...
"""DON'T FORGET TO DELETE THIS PILE OF SHIT
"""
...
class StandaloneNeuron:
    ...
    func _init(n_id: int, curve: float, pos: Vector2, loop: bool, type: int) -> void:
        neuron_id = n_id
...
func load_config(network_name: String) -> void:
    ...
    for neuron_data in network_data["neurons"]:
        var neuron = StandaloneNeuron.new(int(neuron_data["id"]),
                                          neuron_data["curve"],
                                          str2var(neuron_data["pos"]),
                                          neuron_data["loop"],
                                          neuron_data["type"])
